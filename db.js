...
async function updateHeartCount(productId, increment, userInfo = {}) {
    ...
    try {
        ...
        if (!existing) {
            // 如果不存在，创建新文档，服务器端初始值统一为2000
            const serverInitial = 2000; // 服务器端统一从2000开始
            const initialCount = serverInitial + increment;
            ...
            if (result.insertedId) {
                ...
                console.log(`✅ 产品 ${productId} 创建新记录，服务器初始数量: ${newCount} (从2000开始)`);
            } else {
                throw new Error('插入新文档失败');
            }
        } else {
            // 如果存在，服务器端统一从2000开始递增
            // 服务器值与客户端随机初始值相互独立
            const currentCount = existing.count;
            let baseCount = currentCount;
            
            // 服务器端逻辑：统一从2000开始递增
            // 服务器端统一从2000开始递增，与客户端随机值相互独立
            // 如果值等于2000，说明是服务器初始值，从2000开始递增
            // 如果值在2001-2050之间，说明用户已点击，继续递增
            // 如果值在2051-3000之间，可能是旧数据（客户端随机值被错误保存），重置为2000
            // 如果值大于3000，说明用户点击了很多次，继续递增
            // 如果值小于2000，重置为2000
            if (currentCount === 2000) {
                // 服务器初始值2000，从2000开始递增
                baseCount = 2000;
            } else if (currentCount > 2000 && currentCount <= 2050) {
                // 2001-2050之间，说明用户已经点击过，继续递增（服务器值与客户端随机值相互独立）
                baseCount = currentCount;
            } else if (currentCount > 2050 && currentCount <= 3000) {
                // 2051-3000之间，可能是旧数据（客户端随机值被错误保存），重置为2000
                baseCount = 2000;
                console.log(`🔄 产品 ${productId} 服务器当前值 ${currentCount} 可能是旧数据，重置为2000`);
            } else if (currentCount > 3000) {
                // 大于3000，用户点击了很多次，继续递增
                baseCount = currentCount;
            } else if (currentCount < 2000) {
                // 小于2000的异常值，重置为2000
                baseCount = 2000;
                console.log(`🔄 产品 ${productId} 服务器当前值 ${currentCount} 异常，重置为2000`);
            }
    ...
    try {
        ...
        for (const productId of productIds) {
            ...
            if (!existing) {
                ...
                const currentCount = existing.count;
                if (currentCount === 2000) {
                    console.log(`ℹ️ 产品 ${productId} 爱心数量已存在（服务器初始值）: ${currentCount}`);
                } else if (currentCount > 2000 && currentCount <= 2050) {
                    // 2001-2050之间，说明用户已点击，保持不变
                    console.log(`ℹ️ 产品 ${productId} 爱心数量已存在（用户已点击）: ${currentCount}`);
                } else if (currentCount > 2050 && currentCount <= 3000) {
                    // 2051-3000之间，可能是旧数据（客户端随机值被错误保存），重置为2000
                    ...
                    console.log(`🔄 产品 ${productId} 爱心数量已重置: ${currentCount} -> 2000 (旧数据修复)`);
                } else if (currentCount > 3000) {
                    // 大于3000，用户点击了很多次，保持不变
                    console.log(`ℹ️ 产品 ${productId} 爱心数量已存在（用户已多次点击）: ${currentCount}`);
                } else {
                    // 小于2000的异常值，重置为2000
                    await collection.updateOne(
                        { productId: productId },
                    ...
                    console.log(`🔄 产品 ${productId} 爱心数量已重置: ${currentCount} -> 2000 (异常值修复)`);
                }
            }
        }
