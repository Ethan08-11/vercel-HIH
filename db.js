...
async function updateHeartCount(productId, increment, userInfo = {}) {
    ...
    try {
        ...
        if (!existing) {
            ...
            // 如果存在，检查当前值
            // 统一从2000开始递增：如果当前值不是2000的倍数或小于2000，重置为2000
            const currentCount = existing.count;
            let baseCount = currentCount;
            
            // 如果值等于2000，说明是初始值，从2000开始递增
            // 如果值大于2000且是2000的倍数（如2001, 2002, 2003...），说明用户已点击，继续递增
            // 如果值小于2000或大于2000但不是从2000递增的（如旧随机值1835, 2395等），重置为2000
